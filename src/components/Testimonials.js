function Testimonials() {
  ...
  return (
    <section 
      className="py-20 mx-6 md:mx-0 overflow-hidden relative" 
      style={{
        background: 'linear-gradient(to top, #80d2f2 0%, white 100%)' 
      }}
    >
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {testimonials.map((t, i) => (
            <div key={i} className="bg-white p-6 rounded-xl shadow">
              ...
              <p className="font-semibold text-[#04bfad]">- {t.name}</p>
            </div>
          ))}
        </div>
      </div>
